...
def SRGAN_g(t_image, is_train=False, reuse=False):
    ...
    with tf.device('/gpu:0'):
        with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
            ...
            """
            n = Conv2d(n, 250, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, name='n256s1/1')
            n = SubpixelConv2d(n, scale=5, n_out_channel=None, act=tf.nn.relu, name='pixelshufflerx2/1')
            """
...
def residual_block(input, w_init, b_init, g_init, name, is_train=True, reuse=False):

    """
    return a tensorlayer Layer class instead of a tensor 
    (tensor = Layer.outputs; Layer = InputLayer(tensor, name))
    """
    out_channels = input.outputs.get_shape().as_list()[-1]
    
    with tf.variable_scope(name, reuse=reuse):
        x = Conv2d(input, n_filter=out_channels, filter_size=(3,3), strides=(1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='c1')
        x = BatchNormLayer(x, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='bn1' )
        
        x = Conv2d(x, n_filter=out_channels, filter_size=(3,3), strides=(1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='c2')
        x = BatchNormLayer(x, act=tf.identity, is_train=is_train, gamma_init=g_init, name='bn2' )
        
        #x = x + input
        x = ElementwiseLayer(layer=[x, input], combine_fn=tf.add, name='add')
        x.outputs = tf.nn.relu(x.outputs, name='relu2')
        
    return x

def residual_block_first(input, out_channels, strides, w_init,b_init, g_init, name, is_train=True, reuse=False):
    in_channels = input.outputs.get_shape().as_list()[-1]
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if in_channels == out_channels:
            if strides == 1:
                ...
                shortcut = PoolLayer(input, ksize=[1, strides, strides, 1], strides=[1, strides, strides, 1], padding='VALID')
        else:
            shortcut = Conv2d(input, n_filter=out_channels, filter_size=(1,1), strides=(strides, strides), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='shortcut')
        
        x = Conv2d(input, n_filter=out_channels, filter_size=(3,3), strides=(strides, strides), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='c1')
        x = BatchNormLayer(x, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='bn1' )
        
        x = Conv2d(x, n_filter=out_channels, filter_size=(3,3), strides=(1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='c2')
        x = BatchNormLayer(x, act=tf.identity, is_train=is_train, gamma_init=g_init, name='bn2' )
        
        x = ElementwiseLayer(layer=[x, shortcut], combine_fn=tf.add, name='add')
        x.outputs = tf.nn.relu(x.outputs, name='relu2')
        
    return x
    
def SRGAN_d3(input_images, is_train=True, reuse=False):
    """
    resnet-18
    """
    w_init = tf.random_normal_initializer(stddev=0.02)
    ...
    with tf.variable_scope("SRGAN_d", reuse=reuse):
        ...
        # conv1
        net_c1 = Conv2d(net_in, n_filter=64, filter_size=(7,7), strides=(2,2), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='conv1/c')
        net_c1 = BatchNormLayer(net_c1, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='conv1/bn')
        #net_c1 = tf.nn.relu(net_c1)
        #net_c1 = tf.nn.max_pool(net_c1.outputs, [1,3,3,1], [1,2,2,1], 'SAME')
        net_c1 = PoolLayer(net_c1, ksize=[1,3,3,1], strides=[1,2,2,1], padding='SAME', name='conv1/max_pool');
        
        # conv2
        net_c2 = residual_block(net_c1, w_init, b_init, g_init, name='conv2/res_block1', is_train=is_train, reuse=reuse)
        net_c2 = residual_block(net_c2, w_init, b_init, g_init, name='conv2/res_block2', is_train=is_train, reuse=reuse)
        
        # conv3
        net_c3 = residual_block_first(net_c2, out_channels=128, strides=2, w_init=w_init, b_init=b_init, g_init=g_init, name='conv3/res_block1', is_train=is_train, reuse=reuse)
        net_c3 = residual_block(net_c3, w_init, b_init, g_init, name='conv3/res_block2', is_train=is_train, reuse=reuse)
        
        # conv4
        net_c4 = residual_block_first(net_c3, out_channels=256, strides=2, w_init=w_init, b_init=b_init, g_init=g_init, name='conv4/res_block1', is_train=is_train, reuse=reuse)
        net_c4 = residual_block(net_c4, w_init, b_init, g_init, name='conv4/res_block2', is_train=is_train, reuse=reuse)
        
        # conv5
        net_c5 = residual_block_first(net_c4, out_channels=512, strides=2, w_init=w_init, b_init=b_init, g_init=g_init, name='conv5/res_block1', is_train=is_train, reuse=reuse)
        net_c5 = residual_block(net_c5, w_init, b_init, g_init, name='conv5/res_block2', is_train=is_train, reuse=reuse)
        
        # logits
        #with tf.variable_scope("logits", reuse=reuse):
           # x = tf.reduce_mean(net_c5, [1,2])
        x = InputLayer(net_c5.outputs, name='logits/input')
        x.outputs = tf.reduce_mean(x.outputs, [1,2]);
        x = FlattenLayer(x, name='logits/flatten')
        x = DenseLayer(x, n_units=1, act=tf.identity, W_init=w_init, name='logits/dense')
        logits = x.outputs
        x.outputs = tf.nn.sigmoid(x.outputs)
